                                     ...
                                     std::string& error)
{
    ...
    else if (node.target == "torch.ops.higher_order.wrap_with_autocast")
    {
        ...
        if (node.inputs[1].arg.type != EXPORTED_ARGUMENT_SCALAR_TYPE)
            return graph_error(node, "autocast wrapper dtype must be a scalar type", error);
        if (node.inputs[2].arg.type != EXPORTED_ARGUMENT_BOOL || node.inputs[3].arg.type != EXPORTED_ARGUMENT_BOOL)
            return graph_error(node, "autocast wrapper flags must be boolean", error);
    ...
    for (size_t i = 0; i < subgraph.outputs.size(); i++)
    {
        ...
        if (map_tensor_name(node.outputs[i].name, parent_tensor_names, node, output_name, error) != 0)
            return -1;
        if (bind_tensor_name(subgraph.outputs[i].name, output_name, bindings, node, error) != 0)
            return -1;
    }
    ...
    return append_normalized_nodes(subgraph, subgraph_tensor_names, context, error);
}
